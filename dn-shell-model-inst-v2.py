...
def jgetF(u):
    if param == "grad":
        ...
        print(f"No target observable with identifier \"{param}\"")
        return None
...
class Instanton():
    ...
    def optimize(self, lbda, targetObservable=1., mu=0., initialEta=None):
        ...
        if isinstance(self.eta, jnp.ndarray):
            self.eta = self.eta.at[-1,:].set(0.)
        elif isinstance(self.eta, np.ndarray):
            self.eta[-1,:] = 0.
        ...
        for i in range(dim):
            ...
            ax[1].plot(self.eta[:,i], label=f'shell {i}')
        ax[0].set_title('u')
        ax[0].set_ylabel('u')
    ...
    def searchInstantonViaAugmented(self, targetObservable, muMin=np.log10(5.), muMax=np.log10(100.), nMu=8,
                                    initLbda=1., initialEta=None):
        ...
        obsValue, action, lbda, eta, u, ener_diss, dS = self.optimize(initLbda, targetObservable=targetObservable,
                                                                      mu=muList[0], initialEta=initialEta)
        ...
        for j in range(1, nMu):
            obsValue, action, lbda, eta, u, ener_diss, dS = self.optimize(lbda, targetObservable=targetObservable,
                                                                          mu=muList[j], initialEta=eta)
            print(
                "Mu = {}, lambda = {} yields observable = {} and action = {}".format(muList[j], lbda, obsValue, action))
...
if __name__ == '__main__':
    ...
    data_dir = '/Users/rawdata/Downloads/data/{}/nu_{}_c1_{}_c2_{}_obs_{}_date_{}'.format(param, nu, c1, c2, targetObs,
                                                                                            dt_string)
    ...
    n_save = 500
    save_indices = np.linspace(0, nt, n_save).astype(int)
    ...
    if plots:
        ...
        plt.axhline(targetObs, color='red', linestyle='--', linewidth=1,
                    label=f'target $z = {targetObs}$')
        plt.xlabel(r'$t$')
